...
env DEVICE="$(cat /opt/inkbox_device)" LD_LIBRARY_PATH="/lib:system/lib" PATH="/system-bin:/app-bin" QT_FONT_DPI=${QT_FONT_DPI} system/bin/unshare -p -P "${proc_mountpoint}" -- system/lib/ld-musl-armhf.so.1 system/bin/chroot --userspec=user:user "${chroot_directory}" "${application}"
